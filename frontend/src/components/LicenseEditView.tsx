...
const LicenseEditView = () => {
    ...
    React.useEffect(() => {
        const fetchDataLicense = async () => {
            try {
                const { data: { data } } = await axios.get(`${BASE_URL_LICENSE_API}`)
                const findData = data.find((item: any) => item.id === Number(id))
                setKey(findData.key)
    ...
    const requestSaveLicense = async () => {
        ...
        try {
            await axios.put(`${BASE_URL_LICENSE_API}/${id}`, payload)
...
export default LicenseEditView